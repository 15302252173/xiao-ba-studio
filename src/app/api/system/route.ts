...
const LEGACY_PASSWORD_KEY = 'AUTH_PASSWORD';

function getEnvVar(envContent: string, key: string): string | null {
...
function setEnvVar(envContent: string, key: string, value: string): string {
  ...
  if (keyPattern.test(envContent)) {
    return envContent.replace(keyPattern, `${key}=${value}`);
  }
  ...
  try {
    ...
    if (action === 'change_password') {
      ...
      const storedPassword =
        getEnvVar(envContent, ADMIN_PASSWORD_KEY) ??
        getEnvVar(envContent, LEGACY_PASSWORD_KEY);

      if (!storedPassword) {
        return NextResponse.json({ error: 'Mission Control password is not configured' }, { status: 500 });
      }
      ...
      // Update password
      let newEnvContent = setEnvVar(envContent, ADMIN_PASSWORD_KEY, newPassword);

      if (getEnvVar(envContent, LEGACY_PASSWORD_KEY) !== null) {
        newEnvContent = setEnvVar(newEnvContent, LEGACY_PASSWORD_KEY, newPassword);
      }
